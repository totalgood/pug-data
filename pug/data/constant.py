...
DICT_ROMAN2INT = R2I_10 = {'': 0, 'I': 1, 'II': 2, 'III': 3, 'IV': 4,  'V': 5,
                           'VI': 6, 'VII': 7, 'VIII': 8, 'IX': 9}
for num_X in range(1, 4):
    for s, num in R2I_10.items():
        DICT_ROMAN2INT['X' * num_X + s] = 10 * num_X + num
for (prefix, value) in [('XL', 40), ('L', 50)]:
    DICT_ROMAN2INT.update(dict([(prefix + s, value + num) for s, num in R2I_10]))
for num_X in range(1, 4):
    DICT_ROMAN2INT.update(dict([('L' + 'X' * num_X, 50 + 10 * num_X + num) for s, num in R2I_10]))
